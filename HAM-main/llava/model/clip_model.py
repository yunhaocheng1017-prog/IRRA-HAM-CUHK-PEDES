...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None,type=''):
        super().__init__()
        self.width = width
        self.layers = layers
...
def resize_text_pos_embed(posemb, posemb_new):
    # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
    posemb = posemb.unsqueeze(0) 

    print('Resized position embedding from size:{} to size: {}'.format(posemb.shape, posemb_new))
    posemb_grid = posemb.reshape(1, posemb.size(1), -1).permute(0, 2, 1) 
    posemb_grid = F.interpolate(posemb_grid, size=posemb_new, mode='linear')
